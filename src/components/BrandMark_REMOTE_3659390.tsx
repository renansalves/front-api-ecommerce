...
import { useCart, useCartOptional } from "../features/cart/CartContext";
...
import { decodeJwtPayload, getInitialsFromName } from "../utils/user";
...
export function BrandMark({
  ...
  const initials = getInitialsFromName(displayName) || null;
  type JwtPayload = {
    name?: string;
    given_name?: string;
    family_name?: string;
        ...
        <div className="flex items-center justify-end gap-4">
          {showActions && (
            <>
              ...
              >
                <ShoppingCartIcon className="h-5 w-5 text-white" aria-hidden />
                <span className="hidden sm:inline">Carrinho</span>
                {/* Contador (pill) sempre visível; se quiser ocultar quando 0, condicione */}
                <span className="ml-1 inline-flex min-w-[20px] items-center justify-center rounded-full bg-white/20 px-1 text-xs font-semibold">
                  {cartCount}
                </span>
              </Link>
              <Link
              ...
              >
                ...
                <span className="hidden text-sm text-gray-700 sm:inline">{displayName ?? "Minha Conta"}</span>
              </Link>
            </>
          )}
        </div>
